...
<html>
   ...
   <body>
              ...
              <table id="regData" style="margin: 5px auto !important;">
                  ...
                  <tr>
                      ...
                      <td class="w80p" colspan="3">
                        ...
                        <div id="attachedFileArea" style="margin-top:6px;">
                           <p></p>
      ...
      <div id="buttonBtnArea">
         ...
         <button class="blackBtn" onclick="submitForm('<c:url value="/docs/cbnSrvc.do"/>')">목록으로</button> 
      </div>
      ...
      <div id="confirmModal2" class="modal">
          <div class="modal-content">
              ...
              <p>허용되지 않은 파일입니다</p>
            <div id="buttonBtnArea">
                 <button onclick="$('#confirmModal2').css('display','none')" id="confirmNo" class="redBtn">확인</button>
            </div>
          </div>
      </div>
   </body>
...
<script>
     ...
     selectedFile = event.target.files[0];
      ...
      if (selectedFile) {
          ...
          if (!allowedExtensions.includes(fileExt)) {
            $("#confirmModal2").css("display", "block");
              selectedFile.value = ''; // 파일 입력값 초기화
          }
      }
   ...
   $("#confirmYes").on("click", function() {
      const clickedId = $(this).attr("id");
      const actionUrl = clickedId == "deleteBtn"? "<c:url value="/docs/cbnSrvcDel.do"/>" : "<c:url value="/docs/cbnSrvcInsert.do"/>"
       submitForm(actionUrl);
   });

   function submitForm(actionUrl){
         const form = actionUrl == "<c:url value="/docs/cbnSrvcInsert.do"/>" ? document.getElementById("writeFrm") :  document.getElementById("hiddenForm");
