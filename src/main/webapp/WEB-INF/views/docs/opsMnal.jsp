...
<html>
	...
	<body>
		...
		<div id="srchArea">
			<form id="srchForm" method="post" action='<c:url value="/docs/opsMnal.do"/>'>
...
<script>
	$("#regMnalRegBtn").on("click", function() {
		$("#hiddenForm").attr("action", '<c:url value="/docs/opsMnalReg.do"/>');
		$("#hiddenForm").submit();
	});

	$(".btn").on("click", function (event) {www
    ...
    $(".listData").on("click", function () {
	    ...
	    $("#hiddenForm").attr("action", '<c:url value="/docs/opsMnalSpec.do"/>');
        $("#hiddenForm").submit();
    });
</script>
<style>
