...
<html>
	...
	<body>
		...
		<div id="srchArea">
			<form id="srchForm" method="post" action='<c:url value="/docs/safeEdu.do"/>'>
...
<script>
	$("#regMnalRegBtn").on("click", function() {
		$("#hiddenForm").attr("action", '<c:url value="/docs/safeEduReg.do"/>');
		$("#hiddenForm").submit();
	});
    ...
    $(".listData").on("click", function () {
	    ...
	    $("#hiddenForm").attr("action", '<c:url value="/docs/safeEduSpec.do"/>');
        $("#hiddenForm").submit();
    });
</script>
<style>
